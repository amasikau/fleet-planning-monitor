...
async function main() {
  ...
  const centralPaverDemand = await prisma.equipmentDemand.create({
    data: {
      siteId: centralStreet.id,
      stageId: centralAsphaltStage.id,
      ...
      plannedHours: 8,
      priority: 'critical',
      notes: 'Асфальтоукладчик должен работать синхронно с подвозом смеси.',
    },
  });

  const centralRollerDemand = await prisma.equipmentDemand.create({
    data: {
      siteId: centralStreet.id,
      stageId: centralAsphaltStage.id,
      ...
      plannedHours: 8,
      priority: 'critical',
      notes: 'Каток нужен сразу после прохода асфальтоукладчика.',
    },
  });
  ...
  const eastBaseBulldozerDemand = await prisma.equipmentDemand.create({
    data: {
      siteId: eastBaseRoad.id,
      stageId: eastBaseLayerStage.id,
      ...
      plannedHours: 8,
      priority: 'high',
      notes: 'Бульдозер нужен на планировке и распределении грунта.',
    },
  });

  const eastBaseLoaderDemand = await prisma.equipmentDemand.create({
    data: {
      siteId: eastBaseRoad.id,
      stageId: eastBaseLayerStage.id,
      vehicleType: 'loader',
      ...
      plannedHours: 8,
      priority: 'high',
      notes: 'Погрузчик обеспечивает погрузку щебня и подачу материалов.',
    },
  });
  ...
  await prisma.equipmentPlanAssignment.createMany({
    data: [
      ...
      },
      {
        siteId: centralStreet.id,
        stageId: centralAsphaltStage.id,
        demandId: centralPaverDemand.id,
        vehicleId: vogelePaver.id,
        workDate: new Date('2026-06-05T00:00:00.000Z'),
        shift: 'day',
        plannedHours: 8,
        status: 'planned',
        notes: 'Укладка верхнего слоя покрытия на основной карте.',
        createdById: petrov.id,
      },
      {
        siteId: centralStreet.id,
        stageId: centralAsphaltStage.id,
        demandId: centralRollerDemand.id,
        vehicleId: bomagRoller.id,
        workDate: new Date('2026-06-05T00:00:00.000Z'),
        shift: 'day',
        plannedHours: 8,
        status: 'planned',
        notes: 'Уплотнение покрытия вслед за асфальтоукладчиком.',
        createdById: petrov.id,
      },
      {
        siteId: eastBaseRoad.id,
        stageId: eastBaseLayerStage.id,
        demandId: eastBaseBulldozerDemand.id,
        vehicleId: shantuiBulldozer.id,
        workDate: new Date('2026-06-02T00:00:00.000Z'),
        shift: 'day',
        plannedHours: 8,
        status: 'in_progress',
        notes: 'Планировка грунта перед устройством основания.',
        createdById: admin.id,
      },
      {
        siteId: eastBaseRoad.id,
        stageId: eastBaseLayerStage.id,
        demandId: eastBaseLoaderDemand.id,
        vehicleId: amkodorLoader.id,
        workDate: new Date('2026-06-02T00:00:00.000Z'),
        shift: 'day',
        plannedHours: 8,
        status: 'planned',
        notes: 'Погрузка щебня и подача материала на карту работ.',
        createdById: admin.id,
      },
    ],
  });
