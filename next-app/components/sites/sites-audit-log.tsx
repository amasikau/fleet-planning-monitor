...
import type { SiteAuditEntry } from "@/lib/types"
import { SITE_AUDIT_ACTION_LABELS } from "@/lib/types"
...
const actionColors: Record<string, string> = {
  ...
  assign_vehicle: "bg-primary/10 text-primary",
  unassign_vehicle:
...
export function SitesAuditLog({ entries }: { entries: SiteAuditEntry[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      result = result.filter(
        (entry) =>
          entry.siteLabel.toLowerCase().includes(query) ||
          entry.performedBy.toLowerCase().includes(query) ||
          entry.details.toLowerCase().includes(query) ||
          SITE_AUDIT_ACTION_LABELS[entry.action].toLowerCase().includes(query)
      )
    }
    ...
    return [...result].sort((left, right) => {
      ...
      if (sortKey === "timestamp") {
        ...
        compareValue = SITE_AUDIT_ACTION_LABELS[left.action].localeCompare(
          SITE_AUDIT_ACTION_LABELS[right.action],
          "ru"
        )
      } else {
    ...
    <Card className="mx-4 lg:mx-6">
      <CardHeader className="flex-row flex-wrap items-center gap-3">
        <div className="mr-auto flex items-center gap-2">
          ...
          />
          <CardTitle>Журнал действий по объектам</CardTitle>
        </div>
                  ...
                  filtered.map((entry) => (
                    <TableRow key={entry.id}>
                      ...
                      <TableCell>
                        ...
                        >
                          {SITE_AUDIT_ACTION_LABELS[entry.action]}
                        </Badge>
                      </TableCell>
