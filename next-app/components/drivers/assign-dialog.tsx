...
import type { User, DrivingCategory } from "@/lib/types"
import { DRIVING_CATEGORIES, USER_ROLE_LABELS } from "@/lib/types"
...
interface AssignDriverDialogProps {
  ...
  user: User
  onAssign: (data: { userId: string; categories: DrivingCategory[]; documents: { type: string; fileName: string; filePath: string }[] }) => void
}

export function AssignDriverDialog({ open, onOpenChange, user, onAssign }: AssignDriverDialogProps) {
  const [categories, setCategories] = useState<DrivingCategory[]>([])
  ...
  const canAssign = categories.length > 0
  ...
  const handleAssign = () => {
    ...
    onAssign({ userId: user.id, categories, documents })
  }
    ...
    <Dialog open={open} onOpenChange={wrappedOnOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Заполните данные для назначения пользователя водителем
          </DialogDescription>
        </DialogHeader>
        ...
        <div>
          <label className="mb-2 flex items-center gap-1.5 text-sm font-medium">
            ...
            Водительские категории <span className="text-destructive">*</span>
          </label>
          ...
          {categories.length === 0 && (
            <p className="mt-1.5 text-xs text-muted-foreground">Выберите хотя бы одну категорию</p>
          )}
        </div>
