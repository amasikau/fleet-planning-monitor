...
import type { DriverAuditEntry } from "@/lib/types"
import { DRIVER_AUDIT_LABELS } from "@/lib/types"
...
interface DriverAuditLogProps {
  entries: DriverAuditEntry[]
}
...
const actionColors: Record<string, string> = {
  ...
  category_change: "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400",
...
export function DriverAuditLog({ entries }: DriverAuditLogProps) {
  ...
  const filtered = useMemo(() => {
    ...
    if (search) {
      ...
      result = result.filter(
        (e) =>
          ...
          DRIVER_AUDIT_LABELS[e.action].toLowerCase().includes(q)
      )
    }
    ...
    result = [...result].sort((a, b) => {
      ...
      if (sortKey === "timestamp") {
        ...
        cmp = DRIVER_AUDIT_LABELS[a.action].localeCompare(DRIVER_AUDIT_LABELS[b.action], "ru")
      } else {
                    ...
                    <TableRow key={entry.id}>
                      ...
                      <TableCell>
                        <Badge variant="secondary" className={`text-xs font-normal ${actionColors[entry.action] ?? ""}`}>
                          {DRIVER_AUDIT_LABELS[entry.action]}
                        </Badge>
                      </TableCell>
